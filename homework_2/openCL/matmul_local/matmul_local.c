...
int main(void) {
    ...
    int i, j, k;
    int x = X+(LOCAL_SIZE-X%LOCAL_SIZE), n = N+(LOCAL_SIZE-N%LOCAL_SIZE);
    
    int *A = (int*)malloc(sizeof(int)*n*x);
    int *B = (int*)malloc(sizeof(int)*x*n);
    ...
    clut_open_device(&dev, "matmul_local.cl");
    ...
    if (!a_mem_obj) clut_panic("failed to allocate output vec on device memory");
    ...
    err |= clSetKernelArg(kernel, 4, sizeof(int), (void *)&x);
    ...
    for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++) {
            int sum = 0;
            for (k = 0; k < x; k++) sum += A[i*x+k]*B[k*n+j];
